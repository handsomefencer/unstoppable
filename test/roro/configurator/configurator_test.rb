      ...
      describe '.structure' do
        ...
        describe '.smart.env' do
          describe 'dynamic variables' do
            ...
            # Then { expected.each { |e| assert_includes config.smart.env.keys, e } }
            # And  { assert_equal 'omakase', config.smart.env[:main_app_name] }
            # And  { assert_match(/\d.\d./, config.smart.env[:ruby_version]) }
          end
          ...
          describe 'story specific variables' do
            ...
            describe 'must not add mysql smart.env vars to pg story' do \
              Given(:db) { { postgresql: {} } }
              Given { config }

              # Then { assert_includes config.smart.env, :postgres_password }
              # And  { refute config.smart.env.include? :mysql_password }
            end

            describe 'will not add pg smart.env vars to myql story' do
              Given(:db) { { mysql: {} } }
              Given { config }

              # Then { assert_includes config.smart.env, :mysql_password }
              # And  { refute config.smart.env.include? :postgres_password }
            end
          end
        end
      end
